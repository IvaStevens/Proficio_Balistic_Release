...
targetIter = 1;
currentTarget = targetList(targetIter);
...
for i = 1:len(dirs)
    for j = 1:len(forces)
        for k = 1:len(widths)
            for l = 1:len(distances)
                targetList(ind) = targetNode(dirs(i), forces(j), widths(k), distances(l));
                ind = ind + 1;
            end
        end
    end
...
% blah blah blah. documentation
% Struct array.
function [ node ] = targetNode( dir, force, width, distance )
...
% blah blah blah. documentation
function [ targetChanged, nextTarget, rewardable ] = modifyTask(edits, targetIter, nextTarget, rewardable)
...
switch (edits.type)    
    ...
    case 'REWIND' % Do last trial again
        ...
        nextTarget = targetList(targetIter);
        targetChanged = true;
    ...
    case 'TARGET' % Set the target
        ...
        nextTarget = targetNode(dir, dist, force, wid);
        targetChanged = true;
